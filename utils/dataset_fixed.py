...
class CICIDSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = self.labels.iloc[idx, 0]  # Get the first (and only) column
        if self.transform:
            feature = self.transform(feature.values.astype('float32'))
        if self.target_transform:
            label = self.target_transform(label).long()  # Ensure long type for CrossEntropyLoss
        return feature, label


def get_dataset_fixed(data_path: str):
    """Load fixed datasets with proper preprocessing"""
...
def load_data_fixed(data_path: str, batch_size: int):
    """Load fixed training, validation and test set."""

    # Get the datasets
    train_data, val_data, test_data = get_dataset_fixed(data_path=data_path)
